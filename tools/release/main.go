...
func checkGitStatus() error {
	...
	if !strings.Contains(strings.TrimSpace(stdout.String()), "SoeldnerConsult/tofulint-ruleset-google") {
		return fmt.Errorf("remote.origin is not SoeldnerConsult/tofulint-ruleset-google, got %s", strings.TrimSpace(stdout.String()))
	}
	return nil
}
